...
pub struct NextData<'input, 'facet, C = Cooked, I = [u8]>
where
    'input: 'facet,
    I: ?Sized + 'input,
{
...
impl<'input, 'facet, C, I> NextData<'input, 'facet, C, I>
where
    'input: 'facet,
    I: ?Sized + 'input,
{
...
pub type NextResult<'input, 'facet, T, E, C, I = [u8]> =
    (NextData<'input, 'facet, C, I>, Result<T, E>);
...
mod deser_impl {
    ...
    pub fn deserialize<'input, 'facet, T, F>(
        ...
        format: &mut F,
    ) -> Result<T, DeserError<'input, Cooked>>
    where
        T: Facet<'facet>,
        F: Format,
        ...
        'input: 'facet,
    {
        ...
        let result: Result<T, DeserError<'input, Cooked>> = {
            ...
            let wip = match Partial::alloc_shape(T::SHAPE) {
                ...
                Err(e) => {
                    ...
                    return Err(DeserError::new_reflect(e, input, cooked_span));
                }
            };
...
pub fn deserialize_wip<'input, 'facet, F>(
    ...
    input: &'input F::Input<'input>,
    format: &mut F,
) -> Result<HeapValue<'facet>, DeserError<'input, Cooked>>
where
    F: Format,
...
impl<'input, C, I: ?Sized + 'input> StackRunner<'input, C, I>
where
    I: InputDebug,
{
    /// Convenience function to create a DeserError using the original input and last_span.
    fn err(&self, kind: DeserErrorKind) -> DeserError<'input, C> {
    ...
    fn reflect_err(&self, err: ReflectError) -> DeserError<'input, C> {
    ...
    pub fn pop<'facet>(
        ...
        mut wip: Partial<'facet>,
        reason: PopReason,
    ) -> Result<Partial<'facet>, DeserError<'input, C>> {
        trace!(
            "--- STACK has {:?} {}",
            self.stack.green(),
            "(POP)".bright_yellow()
